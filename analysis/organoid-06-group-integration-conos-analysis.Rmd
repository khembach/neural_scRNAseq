...
## We use the sample_id for our cells and the cell line for the organoid cells 
## for integration
sce$sample_id <- ifelse(sce$group_id %in% c("H9", "409b2"), 
                        sce$group_id, as.character(sce$sample_id))
...
prepare_dt <- function(cols_dt, viz_dt, umap_dt, conos_clusters, size = 1e+04){
  ...
  dat$sample_id <- factor(dat$sample_id, levels = c("1NSC", "2NSC", "3NC52",
                                                    "4NC52", "5NC96", "6NC96",
                                                    "H9", "409b2"))
...
label_distr_file <- file.path("output", "conos","conos_label_distr_group_CCA.txt")
```
