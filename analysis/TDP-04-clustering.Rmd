---
title: "Clustering without integration"
...
```{r dr-plots, fig.width = 10, fig.height = 10, warning = FALSE}
...
tdp <- grep(pattern = "\\.TARDBP$", rownames(so), value = TRUE)
ids <- c("sum", "detected", "subsets_Mt_percent", tdp)
for (id in ids) {
    cat("### ", id, "\n")
...
```{r}
saveRDS(so, file.path("output", "so_TDP_04_clustering.rds"))
```
