...
edgeR_res %>% dplyr::filter(gene_name %in%  markers[["up"]]) %>%
  dplyr::mutate(gene_name = factor(gene_name, levels = markers[["up"]])) %>%
  ggplot(aes(x = gene_name, y = logFC, color = FDR<0.05)) + 
...
edgeR_res %>% dplyr::filter(gene_name %in%  markers[["down"]]) %>%
  dplyr::mutate(gene_name = factor(gene_name, levels = markers[["down"]])) %>%
  ggplot(aes(x = gene_name, y = logFC, color = FDR<0.05)) + 
  geom_hline(yintercept = 0, color = "darkgrey")  + 
  geom_point(size = 2) + 
  theme_bw() + 
